	...
	return func(yield func(string, error) bool) {
		for {
			...
			if !ok {
				...
				if !yield("", utils.Err(utils.Fatal, "cannot resolve next node: %s -> ? (code: 1)", gg.NodeIndexID)) {
					return
				}
			}
...
func (gg *GoGenerator) Eval(node tree.Node, isParent ...bool) (string, error) {
	...
	default:
		return "", utils.Err(utils.Warn, "invalid opcode: %d", node.Opcode)
	}
}
	...
	for buf, err := range gg.Next() {
		if err != nil {
			if e, ok := err.(*utils.Error); ok {
				if e.Code == utils.Fatal {
					return "", err
				}
				log.Println(err.Error())
			} else {
				log.Println("Should be using custom error type.")
				return "", err
			}
		}
